...
// Settings
	$save_path = getcwd()."/data/upload/"; // The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
...
// Validate file contents (extension and mime-type can't be trusted)
	...
	*/
	if(!@getimagesize($_FILES[$upload_name]["tmp_name"])) {
		HandleError("Uploaded file is not an image or is corrupted. Please provide valid image.");
		exit(0);
	}
...
// Return output to the browser
	$data = '{stat: "ok", name: "'.$file_name.'"}';
	if(isset($_POST["is_simple"])) {
		echo '<script type="text/javascript">parent.GalleryIns.AddPage.C_uploadSimpleResponse('.$data.')</script>';
	}
	else {
		echo $data;
	}
...
function HandleError($message) {
	$data = '{stat: "fail", message: "'.addslashes($message).'"}';
	//header("HTTP/1.1 500 Internal Server Error");
	if(isset($_POST["is_simple"])) {
		echo '<script type="text/javascript">parent.GalleryIns.AddPage.C_uploadSimpleResponse('.$data.')</script>';
	}
